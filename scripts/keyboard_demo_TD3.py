...
def main(args):
    ...
    if args.vecnorm:
        ...
        evaluation_env = DummyVecEnv([lambda: FlattenObservation(FlattenAction(
            gym.make(config.GYM_ID, key_position=config.KEY_POSITION, key_orientation=config.KEY_ORIENTATION, is_keyboard_demo_env=True,
    ...
    for _ in range(9):
        ...
        while not done:
            ...
            if args.vecnorm:
                ...
                #       .format(reward[0], info[0]["goal"]["distance_to_target"] * 1000, (info[0]["goal"]['percentage_rotation_error']*100)))
            else:
                distance = info["goal"]["distance_to_target"] * 1000
                # print('reward: {:.2f}, distance left: {:.2f} [mm], Rotation Error: {:.2f} %'
                #       .format(reward, info["goal"]["distance_to_target"] * 1000, (info["goal"]['percentage_rotation_error']*100)))
